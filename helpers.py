...
def add_sparsity(distance_matrix, sparsity):
    ...
    # 3. Add edges along the Hamiltonian path
    for i in range(num_nodes - 1):
        mask[path[i], path[i+1]] = True
        mask[path[i+1], path[i]] = True  # Ensure symmetric if needed for TSP

    # 4. Add random edges based on sparsity, excluding the Hamiltonian path edges
    ...
    # 5. Ensure no self-loops (diagonal is False)
